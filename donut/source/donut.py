    ...
    while (running) {
        ...
        for (int j = 0; j < 628; j += 7) {
            for (int i = 0; i < 628; i += 2) {
                ...
                double distance =
                    1 / (sin_i * height * sin_angle1 + sin_j * cos_angle1 + 5);
                double cos_i = calculate_cos(i / 100.0);
                double cos_angle2 = calculate_cos(angle2);
                double sin_angle2 = calculate_sin(angle2);
                double sin_height =
                    sin_i * height * cos_angle1 - sin_j * sin_angle1;

                int x = (int)(40 + 30 * distance *
                    (cos_i * height * cos_angle2 - sin_height * sin_angle2));
                int y = (int)(12 + 15 * distance *
                    (cos_i * height * sin_angle2 + sin_height * cos_angle2));
                int index = x + 80 * y;

                int brightness = (int)(8 *
                    ((sin_j * sin_angle1 - sin_i * cos_j * cos_angle1) *
                    cos_angle2 - sin_i * cos_j * sin_angle1 -
                    sin_j * cos_angle1 - cos_i * cos_j * sin_angle2));
